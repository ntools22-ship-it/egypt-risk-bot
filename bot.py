import feedparser, requests, hashlib, time, os, io
...
SCRAPE_SOURCES = [
    {"id": "independent_breaking", "name": "Independent عربي", "url": "https://www.independentarabia.com/tags/%D8%A7%D9%84%D8%A7%D9%82%D8%AA%D8%B5%D8%A7%D8%AF-%D8%A7%D9%84%D9%85%D8%B5%D8%B1%D9%8A", "tab": "breaking", "base": "https://www.independentarabia.com", "exclude": []},
    {"id": "almal_cbe",            "name": "المال - مركزي",    "url": "https://almalnews.com/tag/%D8%A7%D9%84%D8%A8%D9%86%D9%83-%D8%A7%D9%84%D9%85%D8%B1%D9%83%D8%B2%D9%8A-%D8%A7%D9%84%D9%85%D8%B5%D8%B1%D9%8A/",    "tab": "cbe",     "base": "https://almalnews.com",                    "exclude": []},
]
...
CBE_KW = ["البنك المركزي", "المركزي المصري", "لجنة السياسة النقدية", "سعر الفائدة", "الاحتياطي النقدي", "السياسة النقدية"]
WARNING_KW = ["تعثر", "عجز عن السداد", "توقف عن السداد", "ديون متعثرة", "قروض متعثرة", "محفظة متعثرة", "ديون رديئة", "مخصصات", "شطب ديون", "استرداد ديون", "NPL", "إفلاس", "شهر إفلاس", "إعسار", "تصفية", "حراسة قضائية", "إدارة قضائية", "تعليق النشاط", "وقف الأعمال", "بيع أصول قسري", "حجز على أصول", "حجز على أموال", "دعوى قضائية", "غرامة مالية", "خفض تصنيف", "تخفيض تصنيف", "جدولة ديون", "إعادة جدولة", "أزمة سيولة", "خسائر متراكمة", "مخالفة مالية", "انهيار", "أزمة مالية"]
CREDIT_KW = ["تسهيل ائتماني", "تسهيلات ائتمانية", "قرض", "تمويل", "خط ائتماني", "توريق", "سندات", "صكوك", "قرض مشترك", "تمويل مشترك", "اتفاقية تمويل", "اتفاقية قرض", "ائتمان", "حصلت على تمويل", "وقعت اتفاقية", "منحة قرض", "اعتماد مستندي", "ضمانات بنكية", "رسملة", "بروتوكول تمويل", "مذكرة تفاهم", "تمويل مشروع", "متناهي الصغر", "قرض ميسر"]
DIGEST_PRIORITY = ["warning", "credit", "cbe", "banks", "fx", "global", "breaking", "sector_agri", "sector_industry", "sector_realestate", "sector_energy", "sector_transport", "sector_tech"]
...
def supabase_get_hashes():
    try:
        ...
        r = requests.get(f"{SUPABASE_URL}/rest/v1/news?select=hash&created_at=gte.{since}", headers=sb_headers(), timeout=10)
...
def supabase_save_news(title, url, source_name, tabs, h):
    try:
        r = requests.post(f"{SUPABASE_URL}/rest/v1/news",
            headers={**sb_headers(), "Prefer": "resolution=ignore-duplicates"},
            json={"title": title, "url": url, "source_name": source_name, "tabs": tabs, "hash": h}, timeout=10)
...
def supabase_get_last_24h():
    try:
        ...
        r = requests.get(f"{SUPABASE_URL}/rest/v1/news?select=title,tabs&created_at=gte.{since}&order=created_at.asc", headers=sb_headers(), timeout=15)
        if r.status_code == 200:
            return r.json()
    except Exception as e:
...
def supabase_get_news_for_pdf():
    ...
    try:
        since = (datetime.now(timezone.utc) - timedelta(hours=24)).isoformat()
        r = requests.get(
            f"{SUPABASE_URL}/rest/v1/news?select=title,url,source_name,tabs,created_at&created_at=gte.{since}&order=created_at.asc",
...
def supabase_save_digest(tab_key, tab_label, content, news_count, digest_date):
    try:
        requests.delete(f"{SUPABASE_URL}/rest/v1/digest?tab_key=eq.{tab_key}&digest_date=eq.{digest_date}", headers=sb_headers(), timeout=10)
        r = requests.post(f"{SUPABASE_URL}/rest/v1/digest", headers=sb_headers(),
            json={"tab_key": tab_key, "tab_label": tab_label, "content": content, "news_count": news_count, "digest_date": digest_date}, timeout=10)
        return r.status_code in (200, 201, 204)
    except Exception as e:
...
def format_msg(title, url, source_name, tabs):
    ...
    return "\n".join([f"[{safe_title}]({url})", f"🗂  {tabs_str}", f"📰  {source_name}", "", "━━━━━━━━━━━━━━━━", "🛡 @egypt\\_risk\\_radar"])

def process_item(title, url, source_name, primary_tab, summary, exclude, sent_hashes):
    if not title or not url: return False, sent_hashes
    if not is_arabic(title): return False, sent_hashes
    if any(kw in title for kw in exclude): return False, sent_hashes
    h = make_hash(title)
    if h in sent_hashes: return False, sent_hashes
    tabs = get_tabs(title, summary, primary_tab)
    if not tabs: return False, sent_hashes
...
def fetch_rss(src, sent_hashes):
    ...
    try:
        ...
        for entry in feed.entries[:10]:
            ok, sent_hashes = process_item(
                entry.get("title","").strip(), entry.get("link",""),
                src["name"], src["tab"], entry.get("summary","")[:400],
                src.get("exclude",[]), sent_hashes)
            if ok: count += 1
    except Exception as e:
    ...
    try:
        ...
        for article in soup.find_all("article")[:15]:
            ...
            if h and a:
                ...
                l = a["href"] if a["href"].startswith("http") else src["base"] + a["href"]
                if len(t) > 15: items.append((t, l))
        if not items:
...
def generate_daily_pdf(news_list, now_egypt):
    ...
    pdf.cell(0, 8, ar(f"{date_str}  |  إجمالي: {len(news_list)} خبر في {len(grouped)} تبويبات"), ln=True, align="C")
    pdf.ln(8)
    ...
    for tab in ordered_tabs:
        ...
        for i, item in enumerate(items):
            ...
            # التوقيت
            try:
                dt = datetime.fromisoformat(created_at.replace("Z", "+00:00")) + timedelta(hours=2)
            ...
            title_display = ar(title)
            x_before = pdf.get_x()
            y_before = pdf.get_y()
            pdf.multi_cell(0, 7, f"  {title_display}", align="R", fill=True, link=url if url else "")
...
def run_pdf_report():
    ...
    if send_pdf_to_telegram(pdf_bytes, date_str):
        print(f"✅ تم إرسال PDF ({len(news)} خبر)")
    else:
        print("❌ فشل إرسال PDF")
