...
def title_tokens(text):
    ...
    stop_words = {  
        "مصر", "اليوم", "غدا", "المصري", "المصرية", "في", "من",  
        "على", "عن", "بعد", "قبل", "مع", "الى", "إلى", "هذا",  
        "هذه", "التي", "الذي", "و", "ب", "ل", "ا",  
    }  
    ...
    try:  
        ...
        for script in soup.find_all("script", type="application/ld+json"):  
            try:  
                ...
                for obj in objects:  
                    ...
                    published = obj.get("datePublished") or obj.get("datepublished")  
        ...
        for attr, value in meta_names:  
            ...
            if tag and tag.get("content"):  
                dt = parse_any_datetime(tag.get("content"))  
                if dt:  
                    return dt  
        ...
        print(f"      ⚠️ Date extraction error {url[:80]}: {e}")  
...
def canonical_url(url):
    ...
    tracking_prefixes = ("utm_", "fbclid", "gclid", "ved", "ei")  
    ...
    result = parsed._replace(query=query_string, fragment="").geturl()  
...
def is_duplicate_against_recent(title, url, recent_news):
    ...
    for item in recent_news:  
        ...
        # نفس العنوان  
        if current_norm and old_norm and current_norm == old_norm:  
            return True, "نفس العنوان"  

        # تشابه قوي  
        if not is_update_title(title):  
            if titles_are_probable_duplicate(title, old_title):  
...
def process_item(title, url, source_name, primary_tab, summary, exclude, sent_hashes, recent_news, published_at=None):
    ...
    if published_at is not None:  
        if not is_recent_datetime(published_at, max_age_hours=24):  
...
def extract_listing_items(soup, base, limit=15):
    items = []  
    seen_urls = set()  
    ...
    for article in soup.find_all("article"):  
        ...
        link = absolute_url(base, anchor.get("href"))  
...
def extract_google_news_items(soup, allowed_domains, limit=20):
    items = []  
    seen = set()  
    ...
    for heading in soup.find_all("h3"):  
        ...
        href = google_news_real_url(anchor.get("href"))  

        if not href or not is_allowed_domain(href, allowed_domains):  
            continue  
...
def fetch_scrape(src, sent_hashes, recent_news):
    ...
    try:  
        ...
        if src.get("google_news"):  
            items = extract_google_news_items(soup, src.get("allowed_domains", []), limit=20)  
        else:  
            items = extract_listing_items(soup, src["base"], limit=20)  

        print(f"    🔎 تم العثور على {len(items)} نتيجة في {src['name']}")  

        for title, link in items:  
            published_at = extract_article_date(link)  

            if published_at is None:  
                print(f"    ⚠️ لا يوجد تاريخ موثوق: {title[:70]}")  
                continue  
...
def run():
    ...
    print(f"   {len(recent_news)} خبر في ذاكرة منع التكرار")  
