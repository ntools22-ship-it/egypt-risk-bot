...
def build_daily_report_prompt(grouped, total, date_str):
    ...
    return (
        f"بصفتك خبيراً مصرفياً ومسؤولاً عن إدارة المخاطر في أحد كبرى البنوك المصرية، "
        f"قم بتحليل أخبار اليوم {date_str} التالية وتأثيراتها على القطاع المصرفي.\n"
        f"رصد رادار المخاطر {total} خبراً في {len(grouped)} قطاعات. "
        f"إنذار مبكر: {warning_n} | ائتمان: {credit_n} | بنوك: {banks_n} | مركزي: {cbe_n}\n\n"
        f"الأخبار المصنفة:\n{sections}\n\n"
        f"اكتب التقرير بالهيكل التالي بالضبط:\n\n"
        f"أولاً: الملخص التنفيذي\n"
        f"صورة عامة للمشهد الاقتصادي والمصرفي اليوم في 3-4 جمل.\n\n"
        f"ثانياً: التحليل حسب إدارات قطاع المخاطر\n\n"
        f"1. إدارة مخاطر الائتمان (Credit Risk)\n"
        f"   • التأثير: كيف تؤثر أخبار اليوم على مخاطر الائتمان، PD/LGD، وقرارات منح التمويل؟\n"
        f"   • مستوى الخطر: [عالي / متوسط / منخفض]\n\n"
        f"2. إدارة مخاطر السوق والسيولة (Market & Liquidity Risk)\n"
        f"   • التأثير: تقلبات العملة والفائدة والسيولة وأثرها على مراكز البنك.\n"
        f"   • مستوى الخطر: [عالي / متوسط / منخفض]\n\n"
        f"3. الاستعلامات المصرفية (Banking Inquiries)\n"
        f"   • معلومات تفيد في تقييم ملاءة العملاء والشركات والقطاعات.\n"
        f"   • شركات أو قطاعات تستوجب تدقيقاً إضافياً قبل منح ائتمان.\n\n"
        f"4. مكافحة الاحتيال والمخاطر التشغيلية (Fraud & Operational Risk)\n"
        f"   • أخبار تشير لمخاطر احتيال أو تلاعب أو ضعف حوكمة أو هجمات سيبرانية.\n"
        f"   • مستوى الخطر: [عالي / متوسط / منخفض]\n\n"
        f"5. الحوكمة والالتزام التنظيمي (Compliance & Regulatory Risk)\n"
        f"   • قرارات البنك المركزي أو الرقابة المالية وتأثيرها على السياسات الداخلية للبنك.\n"
        f"   • مستوى الخطر: [عالي / متوسط / منخفض]\n\n"
        f"ثالثاً: إشارات الإنذار المبكر\n"
        f"قائمة بأبرز إشارات الخطر مع مستوى كل منها وتوصية فورية.\n\n"
        f"رابعاً: التوصيات التنفيذية\n"
        f"3-4 توصيات عملية محددة للتطبيق الفوري من قِبل فريق المخاطر.\n\n"
        f"الأسلوب: مهني جداً، دقيق، تحليلي، باللغة العربية الفصحى. بدون مقدمات أو تحيات."
    )
...
def build_weekly_report_prompt(grouped, total, period, daily_counts):
    ...
    return (
        f"بصفتك خبيراً مصرفياً ومسؤولاً عن إدارة المخاطر في أحد كبرى البنوك المصرية، "
        f"قم بتحليل أخبار الأسبوع ({period}) وتأثيراتها على القطاع المصرفي.\n"
        f"رصد رادار المخاطر {total} خبراً في {len(grouped)} قطاعات. "
        f"التوزيع اليومي: {daily_str} | ذروة النشاط: {peak_day}\n"
        f"إنذار: {len(grouped.get('warning',[]))} | ائتمان: {len(grouped.get('credit',[]))} | "
        f"بنوك: {len(grouped.get('banks',[]))} | مركزي: {len(grouped.get('cbe',[]))}\n\n"
        f"الأخبار المصنفة (عينة):\n{sections}\n\n"
        f"اكتب التقرير الأسبوعي بالهيكل التالي:\n\n"
        f"أولاً: ملخص الأسبوع\n"
        f"المشهد العام: هل كان الأسبوع هادئاً أم مقلقاً؟ أبرز 3 أحداث أثّرت في القطاع.\n\n"
        f"ثانياً: تحليل الاتجاهات حسب إدارات المخاطر\n\n"
        f"1. مخاطر الائتمان (Credit Risk)\n"
        f"   • اتجاهات الائتمان الأسبوعية: توسع أم تقلص؟\n"
        f"   • قطاعات تزايدت مخاطرها الائتمانية وتأثيرها على PD/LGD.\n"
        f"   • هل هناك ضغط على خدمة الدين (DSCR) في أي قطاع؟\n"
        f"   • مستوى الخطر الأسبوعي: [عالي / متوسط / منخفض]\n\n"
        f"2. مخاطر السوق والسيولة (Market & Liquidity Risk)\n"
        f"   • تحركات الفائدة والعملة والسيولة وتأثيرها على مراكز البنك.\n"
        f"   • مخاطر العملات غير التقليدية أو التسويات الجديدة.\n"
        f"   • مستوى الخطر الأسبوعي: [عالي / متوسط / منخفض]\n\n"
        f"3. الاستعلامات المصرفية (Banking Inquiries)\n"
        f"   • شركات أو قطاعات برزت في أخبار سلبية تستوجب تدقيقاً مضاعفاً.\n"
        f"   • معلومات تساعد في تقييم الملاءة المالية للعملاء الجدد والقائمين.\n\n"
        f"4. مكافحة الاحتيال والمخاطر التشغيلية (Fraud & Operational Risk)\n"
        f"   • أنماط مستجدة للاحتيال أو التلاعب المالي أو الهجمات السيبرانية.\n"
        f"   • أخبار تشير لضعف في الحوكمة أو الرقابة في أي قطاع.\n"
        f"   • مستوى الخطر الأسبوعي: [عالي / متوسط / منخفض]\n\n"
        f"5. الحوكمة والالتزام التنظيمي (Compliance)\n"
        f"   • قرارات المركزي والرقابة المالية خلال الأسبوع وتأثيرها على السياسات الداخلية.\n"
        f"   • التغييرات التنظيمية التي تستوجب تحديث الإجراءات.\n\n"
        f"ثالثاً: إشارات الإنذار المبكر للأسبوع القادم\n"
        f"أحداث أو مؤشرات يجب مراقبتها مع تقييم مستوى خطر كل منها.\n\n"
        f"رابعاً: اختبارات الضغط المقترحة (Stress Testing)\n"
        f"سيناريوهات يُنصح باختبارها بناءً على أخبار الأسبوع.\n\n"
        f"خامساً: التوصيات التنفيذية الأسبوعية\n"
        f"4-5 توصيات عملية محددة للتطبيق الأسبوع القادم لكل إدارة مختصة.\n\n"
        f"الأسلوب: مهني جداً، تحليلي عميق، باللغة العربية الفصحى. بدون مقدمات أو تحيات."
    )
...
def send_report_to_telegram(title, analysis, date_str, period_icon="📊"):
    ...
    footer = f"\n\n{'━'*20}\n🛡 @egypt\\_risk\\_radar"
    send(header + analysis + footer, parse_mode="Markdown")
